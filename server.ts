...
async function startServer() {
  ...
  app.post('/api/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (category === 'document') {
        ...
        // We use document instead of petDocument based on schema guess or previous calls
  ...
  app.post('/api/pets', async (req, res) => {
    ...
    const pet = await prisma.pet.create({
      data: {
        ...
        name, species, breed, birthDate: birth_date, photoUrl: photo_url, weight, gender, address, city, state, contact
      }
    });
    res.json(pet);
  });
  ...
  app.post('/api/vaccines', async (req, res) => {
    ...
    const vaccine = await prisma.vaccine.create({
      data: { petId: parseInt(pet_id), name, date, nextDue: next_due }
    });
    res.json(vaccine);
  });
  ...
  app.post('/api/services', async (req, res) => {
    ...
    const service = await prisma.service.create({
      data: { providerId: parseInt(provider_id), type, name, description, price: parseFloat(price), location, whatsapp, instagram, photoUrl: photo_url }
    });
    res.json(service);
  });
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        line_items: [{
          price_data: {
            currency: 'brl',
            product_data: {
              name: type === 'RG' ? 'RG Pet Digital' : 'Certidão de Nascimento Pet',
            },
  ...
  // Port is defined at the top of startServer
